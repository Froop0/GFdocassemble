...
class FetchData:
    ...
    def client_info(self, KlientNummer = None):
        ...
        if KlientNummer == None:
            ...
            SQL_str = f"""SELECT HA2.CLIENT_CODE, HA1.NAME_UNO, HA1.ADDRESS1, HA1.CITY, HA1.COUNTRY_CODE 
                            FROM HBM_ADDRESS HA1 
                                LEFT JOIN HBM_CLIENT HA2 ON (HA2.NAME_UNO = HA1.NAME_UNO) 
                                    WHERE HA2.CLIENT_CODE = {KlientNummer} """
            
            client_df = pd.read_sql_query(SQL_str, self.conn)
    ...
    def case_info(self, sagsnummer = None):
        ...
        if sagsnummer == None:
            ...
            SQL_str = f"""SELECT HM1.MATTER_NUMBER, HM1.CLIENT_UNO, HC21.CLIENT_UNO, HC21.SECURITY_ID,
            ...
            case_df = pd.read_sql_query(SQL_str, self.conn)
            
            return case_df
            
    def empl_info(self, MANummer = None, initials = None):
        if MANummer == None:
            ...
            SQL_str = f"""SELECT HP1.INACTIVE, HP1.LOGIN, HP1.PROF, HP1.DEPT, HP1.OFFC,HP1.EMPLOYEE_NAME,
            ...
            emp_df = pd.read_sql_query(SQL_str, self.conn)
